...
### 写作风格要求
- **标题**：
  - ✅ **必须包含表情符号（Emoji）**！例如：【建议收藏✨】、🔥、💻、🚀
  - 标题要直接体现**价值点**。例如：用“这5个效率神器，我真的爱死！”
  - ❌ 禁止平铺直叙。禁止“关于XXX的介绍”。
- **正文**：
  - **分段分段再分段**！每一行或每两行一个段落。
  - **必须大量使用 Emoji**。例如在关键技术点后面加个 ✅，在吐槽处加个 😅，在惊喜处加个 ✨。
...
### 格式特别限制（CRITICAL）
- ❌ **严禁使用 Markdown 加粗**。
- ✅ **重点内容请使用 Emoji 或特殊括号符号**。例如：【重点在这里】。
- ❌ **严禁使用二级、三级标题语法（##, ###）**，直接用 Emoji + 加粗文字替代标题效果。
